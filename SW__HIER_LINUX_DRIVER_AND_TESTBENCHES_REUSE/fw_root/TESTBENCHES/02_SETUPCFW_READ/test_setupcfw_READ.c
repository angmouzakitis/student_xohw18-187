 ...
 * 2017 - 2018 
 *
 * A Complex Read Test can be performed in a relatively simple way 
 * like complex write test. After writing an increasing value to BRAM,
 * a subsequent read operation via the NoC Firewall is performed,
 * and data is compared with the data written. The number of cases
 * is alike the Write Test, i.e. 4096 cases which 1024  are for COMPLEX READ TEST.
 * Thus, in the Complex Read Test, we perform the following steps. For example:
 * 1. Write BRAM memory location via NoC Firewall with a different (always increasing)
 * value default_written_value1 
...
//Port 1 - complex setup with wr accept
		...
		/* 2. Read BRAM memory location via NoC Firewal  */
...
//Port 2 - complex setup with wr accept
...
//Port 3 - complex setup with wr accept
		default_written_value3 += (unsigned int) 1;
		read_value3   = (unsigned int) 65535;

		/* 1. Write BRAM memory location via NoC Firewall  */
                rc1 = accessBramFW(0x0, 3, 3, addr_reg, &default_written_value3); 
...
// Port 4 - complex setup with wr accept
		default_written_value4 += (unsigned int) 1;
		read_value4   = (unsigned int) 65535;

		/* 1. Write BRAM memory location via NoC Firewall  */
                rc1 = accessBramFW(0x0, 4, 4, addr_reg, &default_written_value4);

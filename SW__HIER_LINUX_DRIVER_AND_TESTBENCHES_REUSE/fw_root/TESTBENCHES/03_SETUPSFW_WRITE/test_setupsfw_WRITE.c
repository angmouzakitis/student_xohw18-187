 ...
 * 2017 - 2018
 *
 * 1. Simple Setup NoC Firewall (setupSFW) for a protected memory address
 *    range are specified for each BRAM , by specifying deny read, deny 
 *    write, deny read  & write or accept all. This gives a total of
 ...
 * 2. Perform direct write (accessBram) to each BRAM (at a specific offset 
 *    with a default_written_value) from each input port and subsequently 
 *    verify with subsequent direct read from BRAM that data
 *    (with a read value) has indeed been written. Assuming testing for a 
 *    single BRAM offset, the total number of (orthogonal) cases corresponding 
 *    to input/output port combinations are 4 x 4 = 16, raising the total 
 *    number of unique cases to 16 x 256 = 4096 wich 1024 are for WRITE TEST.
 *      - rc1 = accessBram(0x0, m, 0x00000000, &default_written_value);
 *      - rc2 = accessBram(0x1, m, 0x00000000, &read_value);
 * 3. Wrte BRAM memory location via NoC Firewall with a different 
 *    (always increasing) write_value.
 *       - rc3 = accessBramFW(0x0, m, m, 0x00000000, &write_value); 
 * 4. Read the BRAM value directly (actual_value). The read operation examines
 *    the return code from previous write operation to avoid the possibility
 *     of a write/read reorder.  
 *      - rc4 = accessBram(0x1, m, 0x00000000, &actual_value); 
 ...
 *      - values write_value and actual_value are equal and the setup pattern for 
 *    that port is deny read (accept write) or accept all, or
 *      - values write_value and actual_value are non-equal and setup pattern for 
 *    that port is deny write (accept read) or deny all. 
 */
#include <stdio.h>
#include <stdlib.h>
#include <sys/times.h> 
...
for (i=0; i<4; i++){  
  for (j=0; j<4; j++){
	...
	wBit = (rule & (1 << 5)) >> 5;
	rBit = (rule & (1 << 4)) >> 4;
        setupSFW(1, 0x40000000, 0x40000100, 1, wBit, rBit); // also check [0x0, 0x80] and [0x80, 0x100] -> ECG scenario

	/* Port 2: Get write and read bit from rule for this port and setup FW on this port */
	rule = pattern[j]; //pattern for all
	wBit = (rule & (1 << 5)) >> 5;
	rBit = (rule & (1 << 4)) >> 4;
        setupSFW(2, 0x40001000, 0x40001100, 1, wBit, rBit); // also check [0x0, 0x80] and [0x80, 0x100] -> ECG scenario

	/* Port 3: Get write and read bit from rule for this port and setup FW on this port */
	rule = pattern[k]; //pattern for all
	wBit = (rule & (1 << 5)) >> 5;
	rBit = (rule & (1 << 4)) >> 4;
        setupSFW(3, 0x40002000, 0x40002100, 1, wBit, rBit); // also check [0x0, 0x80] and [0x80, 0x100] -> ECG scenario

	/* Port 4: Get write and read bit from rule for this port and setup FW on this port */
	rule = pattern[l]; //pattern for all
	wBit = (rule & (1 << 5)) >> 5;
	rBit = (rule & (1 << 4)) >> 4;
        setupSFW(4, 0x40003000, 0x40003100, 1, wBit, rBit); // also check [0x0, 0x80] and [0x80, 0x100] -> ECG scenario
	...
	for(m=1; m<5; m++){
		 ...
		 /* 1. Direct default_written_value to each BRAM */ 
                rc1 = accessBram(0x0, m, 0x00000000, &default_written_value); // write 00000000
                do {
		/* 2. Direct Read read_value to each BRAM  */
                        rc2 = accessBram(0x1, m, 0x00000000, &read_value); // write 00000000
                } while (read_value != default_written_value);
		/* 3. Wrie write_value o BRAM memory */
                rc3 = accessBramFW(0x0, m, m, 0x00000000, &write_value); // initially 0;
		/* 4. Read actual_value from BRAM memory */
                if (rc3 == 0) // practically_not_needed: block execution of accessBram read until accessBramFW executed
                rc4 = accessBram(0x1, m, 0x00000000, &actual_value); // intially 65535
